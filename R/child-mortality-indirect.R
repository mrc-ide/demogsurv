...
#' from a DHS Individual Recode file. Unstratified jackknife is used for variance calculation. 
#'
...
#' calc_nqx_indirect(zzir, family = "Princeton-West")
#' 
...
#' calc_nqx_indirect(zzir, ceb = "ceb_m", csurv = "csurv_m",
...
#' calc_nqx_indirect(zzir, ceb = "ceb_f", csurv = "csurv_f",
#'                   family = "Princeton-West", sex = "females")
#' 
#' ## 5q0 by sociodemographic characteristics
#' calc_nqx_indirect(zzir, by=~v102) # by urban/rural residence
#' 
#' @export
#' @md
calc_nqx_indirect <- function(data,
                              by = NULL,
                              clusters=~v021,
                              strata=~v024+v025,
                              weight= "v005",
 ...
 data$classvar = NA
 data$classvar = factor(trunc(c(data$intv-data$dob)/(12*5))*5, levels = seq(15, 45, 5), labels = paste('ma', seq(15, 45, 5), seq(15, 45, 5)+4, sep = "-"))
  
  # Cloning agegr to use in demog_rates
  data$agegr = as.factor(0);  agegr = c(0,1)
  
  # Adding factor variables by which to stratify analysis.
  if(is.null(by)){by <- ~classvar}else{by = update(as.formula(by), ~ . + classvar)}
   ...
   vars <- unique(unlist(lapply(c(by, strata, clusters), all.vars)))
       ...
       if(method == "ma") { # Maternal age -> compute U5MR and produce output 
           ...
           ki_all[1:length(ki_all)] = NA; ti_all[1:length(ti_all)] = NA
           ...
           alpha_all[1:length(alpha_all)] = NA; u5mr_all[1:length(u5mr_all)] = NA
           for(i in c("ma-15-19","ma-20-24","ma-25-29","ma-30-34","ma-35-39","ma-40-44", "ma-45-49") ){
              ...
              ki_jack[1:nrow(ki_jack), 1:ncol(ki_jack)] = NA; ti_jack[1:nrow(ti_jack), 1:ncol(ti_jack)] = NA
              for(i in c("ma-15-19","ma-20-24","ma-25-29","ma-30-34","ma-35-39","ma-40-44", "ma-45-49") ){
              ...
              alpha_jack[1:nrow(alpha_jack),1:ncol(alpha_jack)] = NA; u5mr_jack[1:nrow(u5mr_jack),1:ncol(u5mr_jack)] = NA
              for(i in c("ma-15-19","ma-20-24","ma-25-29","ma-30-34","ma-35-39","ma-40-44", "ma-45-49") ){
                n = as.character(corr[corr$ma == i, "nq0"])
         ...
         data$classvar = factor(data$classvar, levels = seq(0, 30, 5), labels = paste('tsfb', seq(0, 30, 5), seq(0, 30, 5)+4, sep = "-"))
